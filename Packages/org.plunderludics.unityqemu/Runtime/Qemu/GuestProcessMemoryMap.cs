...
public class GuestProcessMemoryMap : ScriptableObject
{
    ...
    public List<Win32X86GuestMemory.PhysicalRange> ToPhysicalRanges()
    {
        var list = new List<Win32X86GuestMemory.PhysicalRange>(ranges.Count);
        for (int i = 0; i < ranges.Count; i++)
        {
            var r = ranges[i];
            list.Add(new Win32X86GuestMemory.PhysicalRange { Start = r.start, Length = r.length });
        }
        return list;
    }
}
